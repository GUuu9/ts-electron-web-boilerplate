...
| **Main (Backend)** | `src/main/features/llm/llm.server.ts` | llama-server HTTP API (Port: 8888)와 직접 통신 |
...
## 🤖 고급 LLM 에이전트 & 세션 관리 기능

### 1. Agentic Workflow (파일 제어)
로컬 LLM이 프로젝트 내 파일을 직접 생성, 수정, 삭제할 수 있습니다.
- **도구**: `WRITE_FILE`, `READ_FILE`, `LIST_FILES`, `DELETE_FILE`
- **경로 권한**: 기본적으로 프로젝트 루트 내부만 접근 가능합니다. 외부 폴더 접근이 필요하면 사이드바의 'Agent Permissions'에서 절대 경로를 추가하십시오.

### 2. 스트리밍 및 컨텍스트 관리
- **실시간 스트리밍**: LLM의 응답이 생성되는 즉시 화면에 출력되어 실시간으로 확인 가능합니다.
- **컨텍스트 길이(n_ctx)**: 모델 구동 시 'Model Settings'에서 컨텍스트 길이를 설정(기본 2048, 최대 32768)할 수 있습니다. 긴 대화가 필요한 경우 이 값을 높이십시오.
- **대화 세션 관리**: 'Conversations' 섹션에서 대화방을 생성, 전환, 삭제할 수 있으며, 대화 내용은 암호화되어 저장됩니다.

### 3. 서버 포트 변경 안내
- `llama-server`가 사용하는 기본 포트가 **8888**로 변경되었습니다. (기존 8080 포트와 메인 앱 충돌 방지)
...
- **포트 충돌**: `llama-server`는 8888 포트를 사용합니다. 해당 포트를 사용하는 다른 프로세스가 있으면 실행되지 않습니다.
